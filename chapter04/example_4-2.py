# 例4.2：ジャックのレンタカー会社ジャックは全国規模のレンタカー会社の二つの支所のマネージャーである。
# 毎日何人かの客がそれぞれの支所に車を借りにくる。ジャックは貸せる車があれば1台貸し、本社から10ドルもらえる。
# もしその支所に車がない場合には、貸すことはできない。
# 車は返却された次の日から貸せるようになる。
# 車が必要な場所にあるように、ジャックは1台ごとに2ドルかけて、夜の間に二つの支所の間で移動させることができる。
# 各支所での1日の車の要求台数と返却数は、ポアソン分布に従う確率変数であると仮定する。
# すなわち。台数がnである確率は（lambda^n / n! * e^-lambda）となる．ただし入は期待値を表す．
# 2箇所での車の要求台数の入がそれぞれ3,4であり、返却台数の入はそれぞれる、2であるとする。
# 問題を少し簡単にするため、各支所には20台よりも多く車は置けないとする（20台を超える車は本社に返却され、この問題では考えなくてよいとする）.
# また、夜の間に移動できる車は最大で5台までとする。
# 割引率は0.9として連続的な有限マルコフ決定過程とする。
# 時間ステップは1日であり、状態は各支所での1日の終わりの車の台数、行動は夜の間に移動させる車の台数である。
# 図4.2は決して車を動かさないという方策から始めた、方策反復によって求めた方策の系列である。
# ジャックのレンタカー問題の図と図4.1の例が示すように、方策反復は非常に少ない数の反復で収束することが多い。
# 図4.1の左下の図はすべての行動を同じ確率でとるランダム方策の価値関数であり、右下の図はその価値関数に対してグリーディな方策である。
# 方策改善定理はこれらの方策が元のランダム方策よりもよいことを保証する。
# だがこの場合、これらの方策はただよいだけでなく最適であるため、最小ステップで終端状態に向かう。
# この例では、方策反復は1回の反復で最適方策を見つけることができる.
