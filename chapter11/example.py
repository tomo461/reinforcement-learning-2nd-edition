...
PROJECTION_MAT = (
    np.matrix(FEATURES)
    * np.linalg.pinv(
        np.matrix(FEATURES.T) * STATE_DISTRIBUTION_MAT * np.matrix(FEATURES)
    )
...
def semi_gradient_off_policy_TD(state, theta, alpha):
    ...
    delta = (
        REWARD
        + DISCOUNT * np.dot(FEATURES[next_state, :], theta)
...
def TDC(state, theta, weight, alpha, beta):
    ...
    theta += (
        ...
        * rho
        * (
            delta * FEATURES[state, :]
            - DISCOUNT * FEATURES[next_state, :] * np.dot(FEATURES[state, :], weight)
        )
    )
    weight += (
        beta * rho * (delta - np.dot(FEATURES[state, :], weight)) * FEATURES[state, :]
    )
    return next_state
...
def expected_TDC(theta, weight, alpha, beta):
    for state in STATES:
        ...
        delta = (
            REWARD
            + DISCOUNT * np.dot(FEATURES[LOWER_STATE, :], theta)
            - np.dot(FEATURES[state, :], theta)
        )
        ...
        expected_update_theta = (
            ...
            * (
                ...
                * np.dot(weight, FEATURES[state, :])
            )
        )
        theta += alpha * expected_update_theta
        expected_update_weight = (
            1.0
            / len(STATES)
            * BEHAVIOR_SOLID_PROBABILITY
            * rho
            * (delta - np.dot(weight, FEATURES[state, :]))
            * FEATURES[state, :]
        )
    ...
    for state in STATES:
        for next_state in STATES:
            if next_state == LOWER_STATE:
                bellman_error[state] += (
                    REWARD
                    + DISCOUNT * np.dot(theta, FEATURES[next_state, :])
                    - np.dot(theta, FEATURES[state, :])
                )
